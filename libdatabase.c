...
static const char *deleteSearch = "DELETE FROM `SEARCH` WHERE `KEY` is ?";
...
static size_t compareDomains(gchar *req, const unsigned char *dbr, char implicit) {
    size_t score = 0;
    if (implicit) {
...
static size_t compute_rule(gchar *reqf, gchar *reqt, const unsigned char *dbrf, const unsigned char *dbrt, int allow) {
...
extern void sql_download_write(const char *page, const char *url, const char *file) {
    sqlite3 *db;
    //char * zErrMsg = 0;
    int rc;
    if (!url)
...
extern char sql_search_write(const char *key, const char *url, const char *name) {
    sqlite3 *db;
    int rc;
    SEARCHDIR(searchdir);
