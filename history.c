...
enum {
    ADDRESS_COLUMN, TITLE_COLUMN, VISITED_COLUMN, N_COLUMNS
};
...
static int treeIter(void *store, int count, char **data, char **columns) {
    ...
    if (count == 3) {
        ...
        gtk_tree_store_set(store, &iter, 0, data[0], 1, data[1], 2, s ? s : "Unknown", -1);
        g_free(s);
        g_date_time_unref(d);
    }
    return 0;
...
void c_history_url(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, void *v) {
    gchar *str_data;
...
gboolean c_history_url_tab(GtkTreeView *tree, GdkEventButton *event, void *v) {
    if (event->type == GDK_BUTTON_RELEASE) {
        if (event->button == 2) { //Middle click
            gchar *str_data;
            GtkTreePath *path;

            if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(tree), (gint) event->x, (gint) event->y, &path, NULL, NULL,
                                              NULL)) {
                GtkTreeIter iter;
...
static void search_entry_change(GtkWidget *e, GtkTreeModelFilter *f) {
    ...
    gtk_adjustment_set_value(gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(G_scrollWin)), 0.0);
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(G_HISTORY)),
                          gdk_cursor_new_from_name(gdk_display_get_default(), "default"));
}

static gboolean hisstrstr(GtkTreeModel *model, GtkTreeIter *iter, void *v) {
...
static void load_data_thread(GTask *task, gpointer source_object, gpointer store, GCancellable *cancellable) {
...
void history_load_data_async(void *self, GCancellable *cancellable, GAsyncReadyCallback callback) {
    GTask *task;
...
GtkTreeStore *history_load_data_finish(void *self, GAsyncResult *result, GError **error) {
    g_return_val_if_fail(g_task_is_valid(result, self), NULL);

    return g_task_propagate_pointer(G_TASK(result), error);
}

static void history_load_data_result(GObject *obj, GAsyncResult *res, gpointer user_data) {
...
extern void InitHistoryWindow(void *v) {
    ...
    store = NULL;
    G_search = NULL;
    G_HISTORY = (GtkWindow *) gtk_window_new(GTK_WINDOW_TOPLEVEL);
    ...
    gtk_scrolled_window_set_min_content_width(GTK_SCROLLED_WINDOW(G_scrollWin), 320);
    gtk_scrolled_window_set_min_content_height(GTK_SCROLLED_WINDOW(G_scrollWin), 240);
    ...
    column = gtk_tree_view_column_new_with_attributes("URL", renderer, "text", ADDRESS_COLUMN, NULL);
    ...
    gtk_tree_view_column_set_resizable(column, TRUE);
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes("Last Visited", renderer, "text", VISITED_COLUMN, NULL);
    ...
    * could use any other GtkTreeModel */
    ...
    filtered = GTK_TREE_MODEL_FILTER(gtk_tree_model_filter_new(GTK_TREE_MODEL(store), NULL));

    sort = GTK_TREE_MODEL_SORT(gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(filtered)));
    sortable = GTK_TREE_SORTABLE(sort);

    gtk_tree_sortable_set_sort_column_id(sortable, VISITED_COLUMN, GTK_SORT_DESCENDING);

    gtk_tree_view_set_model(GTK_TREE_VIEW(tree), GTK_TREE_MODEL(sortable));

    g_object_unref(G_OBJECT(store));
    g_object_unref(G_OBJECT(filtered));
    g_object_unref(G_OBJECT(sort));

    g_signal_connect(G_HISTORY, "destroy", G_CALLBACK(c_destroy_window), NULL);
    g_signal_connect(searchEntry, "activate", G_CALLBACK(search_entry_change), filtered);
    g_signal_connect(tree, "row-activated", G_CALLBACK(c_history_url), v);
    g_signal_connect(tree, "button-release-event", G_CALLBACK(c_history_url_tab), v);

    gtk_tree_model_filter_set_visible_func(filtered, (GtkTreeModelFilterVisibleFunc) hisstrstr, NULL, NULL);
    ...
    gtk_widget_show_all((GtkWidget *) G_HISTORY);
    g_signal_connect(G_HISTORY, "destroy", G_CALLBACK(c_on_destroy), NULL);
}
