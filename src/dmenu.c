...
static void c_goto_dial(GtkMenuItem *mi, GdkEventButton *e, void *v) {
    ...
    char *url = sql_speed_dial_get_by_name(gtk_menu_item_get_label(mi));
    if (!url)
        return;
...
static int gotoIter(void *store, int count, char **data, char **columns) {
    if (count == 3) {
        ...
        g_signal_connect(mi, "button-release-event", G_CALLBACK(c_goto_dial), NULL);
...
void c_hide_gotoMenu(GtkMenuItem *mi, PlancWindow *v) {
    struct call_st *c = planc_window_get_call(v);
    GList *list = gtk_container_get_children((GtkContainer *) c->menu->gotoMenu);
    for (GList *l = list; l != NULL; l = l->next)
        gtk_widget_destroy(l->data);
}

void c_select_tabsMi(GtkWidget *w, struct dpco_st *dp) {
    gtk_notebook_set_current_page(dp->call->tabs, gtk_notebook_page_num(dp->call->tabs, dp->other));
}
...
void c_hide_tabsMenu(GtkMenuItem *mi, PlancWindow *v) {
    struct call_st *c = planc_window_get_call(v);
    GList *list = gtk_container_get_children((GtkContainer *) c->menu->tabsMenu);
...
static GtkRadioMenuItem *add_tabMi(gint it, GSList *list, struct call_st *c) {
    GtkWidget *l = gtk_bin_get_child(
                   GTK_BIN(gtk_notebook_get_tab_label(c->tabs, gtk_notebook_get_nth_page(c->tabs, it))));
    GtkRadioMenuItem *n = (GtkRadioMenuItem *) gtk_radio_menu_item_new(list);
    gtk_menu_item_set_label((GtkMenuItem *) n, gtk_label_get_text((GtkLabel *) l));
    if (gtk_notebook_get_nth_page(c->tabs, it) == WK_CURRENT_TAB_WIDGET(c->tabs))
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(c->menu->tabsMenu), (GtkWidget *) n);

    g_signal_connect(n, "button-release-event", G_CALLBACK(c_onclick_tabsMi), dp);
...
void t_tabs_menu(PlancWindow *v, gboolean b) {
    ...
    if (b) {
        ...
        gtk_menu_shell_insert(GTK_MENU_SHELL(c->menu->menu), c->menu->tabsMh, 4);
        gtk_widget_show_all(c->menu->menu);
        c->menu->tabsMenu = gtk_menu_new();
        gtk_widget_add_events(c->menu->tabsMenu, GDK_KEY_PRESS_MASK);
        gtk_menu_item_set_submenu((GtkMenuItem *) c->menu->tabsMh, c->menu->tabsMenu);
        g_signal_connect(c->menu->tabsMenu, "show", G_CALLBACK(c_show_tabsMenu), v);
        g_signal_connect_after(c->menu->tabsMenu, "hide", G_CALLBACK(c_hide_tabsMenu), v);
    } else {
        gtk_container_remove(GTK_CONTAINER(c->menu->menu), c->menu->tabsMh);
        c->menu->tabsMh = NULL;
    }
}
