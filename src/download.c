...
static int treeIter(void *store, int count, char **data, char **columns) {
    GtkTreeIter iter;
    if (count == 3) {
        ...
        gtk_tree_store_set(store, &iter, PAGE_COLUMN, purl, DURL_COLUMN, data[1], FILE_COLUMN, data[2], NAME_COLUMN,
                           file, STAT_COLUMN, "Complete", PROG_COLUMN, 100, -1);
    }
    return 0;
}

static GtkTreeIter *
addDownloadEntry(const gchar *file, const gchar *name, const gchar *durl, const gchar *purl, const gchar *stat,
                 void *v) {
    InitDownloadWindow();
    GtkTreeIter *r = malloc(sizeof(GtkTreeIter));
    gtk_tree_store_append(G_store, r, NULL);

    gtk_tree_store_set(G_store, r, PAGE_COLUMN, purl, DURL_COLUMN, durl, FILE_COLUMN, file, NAME_COLUMN, name,
                       STAT_COLUMN, "Downloading", PROG_COLUMN, 0, -1);

    return r;
}

static gboolean dlstrstr(GtkTreeModel *model, GtkTreeIter *iter, void *v) {
...
static void c_download_finished(WebKitDownload *d, GtkTreeIter *iter) {
    guint64 len = webkit_uri_response_get_content_length(webkit_download_get_response(d));
    gchar *l = NULL;
    if (len) {
        l = g_strdup_printf("Downloaded: %"
                            G_GUINT64_FORMAT
                            "/%"
                            G_GUINT64_FORMAT, webkit_download_get_received_data_length(d), len);
    } else {
        l = g_strdup_printf("Downloaded: %"
                            G_GUINT64_FORMAT, webkit_download_get_received_data_length(d));
    }
    gtk_tree_store_set(G_store, iter, STAT_COLUMN, l, PROG_COLUMN, 100, -1);
    free(l);
    free(iter);
}

static void c_download_progress(WebKitDownload *d, guint pro, GtkTreeIter *iter) {
    guint64 len = webkit_uri_response_get_content_length(webkit_download_get_response(d));
    gchar *l = NULL;
    if (len) {
        l = g_strdup_printf("Downloading: %"
                            G_GUINT64_FORMAT
                            "/%"
                            G_GUINT64_FORMAT, webkit_download_get_received_data_length(d), len);
    } else {
    ...
    gtk_tree_store_set(G_store, iter, STAT_COLUMN, l, PROG_COLUMN, i, -1);
    free(l);
}

static const gchar *getDownloadPageUrl(WebKitDownload *d) {
    const gchar *p = webkit_web_view_get_uri(webkit_download_get_web_view(d));
    if (!p || strcmp(p, "") == 0)
        return p;
    const gchar *a = webkit_uri_request_get_uri(webkit_download_get_request(d));
    return a;
}

static void c_download_destination_created(WebKitDownload *d, gchar *u, void *v) {
    ...
    g_signal_connect(d, "received-data", G_CALLBACK(c_download_progress), t);
    g_signal_connect(d, "finished", G_CALLBACK(c_download_finished), t);
}
...
static gboolean c_download_save_as(WebKitDownload *d, gchar *fn, gchar *fp) {
    GtkWidget *dialog;
    GtkFileChooser *chooser;
    gint res;
    GtkWindow *w = gtk_application_get_active_window(G_APP);
    dialog = gtk_file_chooser_dialog_new("Save File", (GtkWindow *) w, GTK_FILE_CHOOSER_ACTION_SAVE, ("_Cancel"),
    ...
    if (fp)
        ...
        gtk_file_chooser_set_current_folder(chooser, g_get_user_special_dir(G_USER_DIRECTORY_DOWNLOAD));

    res = gtk_dialog_run(GTK_DIALOG(dialog));
    if (res == GTK_RESPONSE_ACCEPT) {
        webkit_download_set_destination(d, gtk_file_chooser_get_uri(chooser));
...
static void
progress_cell_data_func(GtkTreeViewColumn *c, GtkCellRenderer *renderer, GtkTreeModel *model, GtkTreeIter *iter,
                        gpointer p) {
    gchar *stat;
    gint *slider;

    gtk_tree_model_get(model, iter, STAT_COLUMN, &stat, PROG_COLUMN, &slider, -1);

    g_object_set(renderer, "value", slider, NULL);
    g_object_set(renderer, "text", stat, NULL);
}

static gboolean c_download_prompt(WebKitDownload *d, gchar *fn, PlancWindow *v) {
    InitDownloadWindow();
    gchar *t = NULL;
    gchar *f = NULL;
    if (fn == NULL || strcmp(fn, "") == 0) {
        ...
        if (f)
            t = g_strconcat("Download request '", f, "' - Plan C", NULL);
        else
    ...
    GtkWidget *DownloadPrompt = gtk_dialog_new_with_buttons(t, G_DOWNLOAD, GTK_DIALOG_DESTROY_WITH_PARENT, _("_Save"),
                                GTK_RESPONSE_OK, _("Save _As"), GTK_RESPONSE_APPLY,
                                _("_Cancel"), GTK_RESPONSE_CANCEL, NULL);

    g_free(t);

    const gchar *absdl = g_get_user_special_dir(G_USER_DIRECTORY_DOWNLOAD);
    ...
    switch (g_settings_get_boolean(G_SETTINGS, "download-domain")) {
        default:
            abspth = g_build_filename(fdlp, getDomainName(getDownloadPageUrl(d)), NULL);

            if (g_file_test(abspth, G_FILE_TEST_IS_DIR) || g_mkdir(abspth, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == 0)
    ...
    GtkWidget *dbox = gtk_dialog_get_content_area(GTK_DIALOG(DownloadPrompt));

    gchar *str_name = g_strconcat("Name: ", f, NULL);
    gchar *str_puri = g_strconcat("Page: ", webkit_uri_request_get_uri(webkit_download_get_request(d)), NULL);
    gchar *str_adir;
    if (g_file_test(absdir, G_FILE_TEST_EXISTS)) {
        str_adir = g_strconcat("Save to: ", absdir, " (File exists)", NULL);
        gtk_dialog_set_response_sensitive(GTK_DIALOG(DownloadPrompt), GTK_RESPONSE_OK, FALSE);
    } else
...
void c_download_start(WebKitWebContext *wv, WebKitDownload *dl, void *v) {
    webkit_download_set_allow_overwrite(dl, TRUE);
    g_signal_connect(dl, "decide-destination", G_CALLBACK(c_download_prompt), NULL);
    g_signal_connect(dl, "created-destination", G_CALLBACK(c_download_destination_created), NULL);
}

void c_download_dir(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, void *v) {
    gchar *str_data;
    ...
    if (gtk_tree_model_get_iter(model, &iter, path)) {
        gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, 5, &str_data, -1);
        openFile(str_data);
    }
}
...
void InitDownloadWindow() {
    ...
    G_search = NULL;
    G_DOWNLOAD = (GtkWindow *) gtk_window_new(GTK_WINDOW_TOPLEVEL);
    ...
    gtk_scrolled_window_set_min_content_width(GTK_SCROLLED_WINDOW(scrollWin), 320);
    gtk_scrolled_window_set_min_content_height(GTK_SCROLLED_WINDOW(scrollWin), 240);
    ...
    * could use any other GtkTreeModel */
    G_store = gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT,
    ...
    filtered = GTK_TREE_MODEL_FILTER(gtk_tree_model_filter_new(GTK_TREE_MODEL(G_store), NULL));

    gtk_tree_model_filter_set_visible_func(filtered, (GtkTreeModelFilterVisibleFunc) dlstrstr, NULL, NULL);

    sort = GTK_TREE_MODEL_SORT(gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(filtered)));
    sortable = GTK_TREE_SORTABLE(sort);

    gtk_tree_sortable_set_sort_column_id(sortable, FILE_COLUMN, GTK_SORT_DESCENDING);
    ...
    * reference */
    ...
    column = gtk_tree_view_column_new_with_attributes("File", renderer, "text", NAME_COLUMN, NULL);
    ...
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes("Download Page", renderer, "text", PAGE_COLUMN, NULL);
    ...
    column = gtk_tree_view_column_new_with_attributes("Status", renderer, "text", STAT_COLUMN, NULL);
    ...
    g_signal_connect(G_DOWNLOAD, "destroy", G_CALLBACK(c_destroy_window), NULL);
    g_signal_connect_after(searchEntry, "search-changed", G_CALLBACK(search_entry_change), filtered);
    g_signal_connect(tree, "row-activated", G_CALLBACK(c_download_dir), NULL);
    ...
    g_signal_connect(G_DOWNLOAD, "destroy", G_CALLBACK(c_on_destroy), NULL);
}
