...
static gboolean c_webpage_context(WebKitWebPage * wp
	,WebKitContextMenu * cm, WebKitWebHitTestResult *hr, void * v)
{
	...
	js_value = jsc_context_evaluate (js_context
		,"window.getSelection().toString();", -1);
	if (!jsc_value_is_null (js_value)
	&& !jsc_value_is_undefined (js_value))
	...
	g_variant_builder_add (&builder, "{sv}", "SelectedText"
		,g_variant_new_string (g_strstrip (string)));
	webkit_context_menu_set_user_data (cm
	...
	WebKitWebPage * wp, gpointer user_data)
{
	...
	g_signal_connect (wp, "context-menu"
		,G_CALLBACK (c_webpage_context), NULL);
}
